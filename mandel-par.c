...
main (int argc, char *argv[]) 
{
	...
	local_x_size = x_size; 
	local_y_size = y_size/number_of_p;
	...
	compute ( &local_pict , n_iter, local_x_min, local_x_max, local_y_min, local_y_max);
		sprintf(path, "mandel%d.ppm", my_rank);
		printf("%s\n", path);
		save_picture (&local_pict, path);
	MPI_Gather(local_pict.pixels, local_x_size*local_y_size, MPI_CHAR , pict.pixels, local_x_size*local_y_size, MPI_CHAR, root, MPI_COMM_WORLD);
	if ( my_rank == root) 
